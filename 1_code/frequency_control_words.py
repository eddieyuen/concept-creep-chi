...
control_words_file = '/Users/kawaiyuen/nlpworkshop/concept-creep-chi/0_data/wordlist/control_words.json'

# Path to the output CSV file
output_csv = '/Users/kawaiyuen/nlpworkshop/concept-creep-chi/2_pipeline/preprocessed/frequency_control_words.csv'
...
with open(control_words_file, 'r') as f:
    control_words_data = json.load(f)

# Prepare the header and data for the CSV file
header = ['Year'] + [concept[0] + '_freq' for concept in control_words_data.values()]  
data = []

# Extract the frequencies for each token and year
for year in range(1979, 2024):
    row = [year]
    for token_key in control_words_data:
        token = control_words_data[token_key][0]  # Get the first item from the list of tokens
