...
concepts_columns = [concept[0] + '_relativefreq' for concept in concepts_data.items()]

# Specify file path
relativefreq_concepts_path = '/Users/kawaiyuen/nlpworkshop/concept-creep-chi/2_pipeline/MAIN-FULL_sb-win-size-5.xlsx'
# Read relative frequency data
data = pd.read_excel(relativefreq_concepts_path)
years = data['Year']
...
for i, concepts_column in enumerate(concepts_columns):
    # Get the Chinese translation for the current concept
    chinese_concept = concepts_data.get(concepts_column.split("_")[0])[0]
    ...
    ax.plot(years, relativefreq, label=f'{concepts_column.split("_")[0]} ({chinese_concept})', color=color)
...
for i, concepts_column in enumerate(concepts_columns):
    ...
    # Plot relative frequency values for each concept with a unique color
    relativefreq = data[concepts_column]
    ax.plot(years, relativefreq, alpha=0.7, linestyle='dashed',linewidth=0.6, color=color)
    # Calculate and plot moving average with window=5
    moving_average = data[concepts_column].rolling(window=5, min_periods=1).mean()
    ax.plot(years, moving_average, label=f'{concepts_column.split("_")[0]} ({chinese_concept})', linewidth=2, color=color)
